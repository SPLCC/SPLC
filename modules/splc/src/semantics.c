...
static void register_typedef(const ast_node node)
{
    ...
    {
        lut_insert(current_trans_unit->global_symtable, (const char *)node->val, SPLE_TYPEDEF, SPLE_NULL, NULL, node, node->location);
    }
    for (int i = 0; i < node->num_child; ++i)
    {
        switch (node->children[i]->type)
        ...
        case SPLT_DIR_DEC:
            ...
            break;
        case SPLT_ID:
            lut_insert(current_trans_unit->global_symtable, (const char *)node->children[i]->val, SPLE_TYPEDEF, SPLE_NULL, NULL, node, node->location);
            break;
        default:
            break;
        }
...
void sem_analyze(splc_trans_unit tunit)
{
    ...
    ast_sem_search(tunit->root, tunit, 0, SPLE_NULL, SPLE_NULL, NULL, 0);
    ast_sem_expr(tunit->root, tunit->root);
}